...
# falls back to a hardcoded default when it is not passed (e.g. CodeQL autobuild,
# which runs a bare gradle with no -Dversion). Keeping that default equal to
# VERSION.json stops it drifting behind the real version and requesting a
# common-utils artifact (${wazuh_version}.${revision}) that was never published.
# Arguments:
#   $1 - version
# ====
